...
export class AppComponent {
  ...
  highPriorityOperationKeys: typeof HighPriorityOperationKeys =
    HighPriorityOperationKeys;
  mediumPriorityOperationKeys: typeof MediumPriorityOperationKeys =
    MediumPriorityOperationKeys;
  lowPriorityOperationKeys: typeof LowPriorityOperationKeys =
    LowPriorityOperationKeys;
  ...
  private calculateEquation(): void {
    try {
      let flatExpression: string = this.calculateExpressionInParenthesis(
        this.equation
      );
  ...
  private calculateExpressionInParenthesis(expression: string): string {
    const parenthesisPattern: RegExp = new RegExp(
      `\\${DelimiterKeys.leftParenthesis}([0-9]|\\${
        DelimiterKeys.comma
      }|\\${Object.values(this.highPriorityOperationKeys).join(
        '|\\'
      )}|\\${Object.values(this.mediumPriorityOperationKeys).join(
        '|\\'
      )}|\\${Object.values(this.lowPriorityOperationKeys).join('|\\')})+\\${
        DelimiterKeys.rightParenthesis
      }`
    );
    expression = this.performReplacement(
      ...
      (substr) =>
        this.calculateExpression(
          substr.replace(
            new RegExp(
              `\\${DelimiterKeys.leftParenthesis}|\\${DelimiterKeys.rightParenthesis}`,
              'g'
            ),
            ''
          )
  ...
  private calculateExpression(expression: string): string {
    ...
    const mediumPriorityOperationsPattern: RegExp = new RegExp(
      `[0-9]+(\\${DelimiterKeys.comma}[0-9]+)?(\\${Object.values(
        this.mediumPriorityOperationKeys
      ).join('|\\')})[0-9]+(\\${DelimiterKeys.comma}[0-9]+)?`
    );
    expression = this.performReplacement(
      expression,
